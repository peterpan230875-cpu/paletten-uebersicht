...
app.use((req, res, next) => {
    console.log(`📨 ${req.method} ${req.path} - Full URL: ${req.originalUrl}`);
    next();
});
...
// ============================================
// SETUP MAIN ROUTE (before API routes)
// ============================================
console.log('⚙️ Setting up main route handler (early)...');
const publicDir = path.join(path.dirname(__filename), 'public');
const indexFile = path.join(publicDir, 'index.html');
console.log('📁 Public Dir:', publicDir);
console.log('📄 Index File:', indexFile);

// First, try using app.use for root with explicit path
app.use('/', (req, res, next) => {
    console.log('🔹 Middleware handler called for path:', req.path, 'URL:', req.url);
    if (req.path === '/') {
        console.log('🔸 Serving index file from middleware');
        res.sendFile(indexFile, (err) => {
            if (err) {
                console.error('❌ Error in middleware sendFile:', err.message);
                next();  // Pass to next middleware if error
            }
        });
    } else {
        console.log('🔸 Passing to next middleware for:', req.path);
        next();
    }
});

console.log('✅ Main route handler registered (early)');
...
// ============================================
console.log('⚙️ Setting up static middleware with dir:', publicDir);
console.log('   Files in publicDir:', require('fs').readdirSync(publicDir));
